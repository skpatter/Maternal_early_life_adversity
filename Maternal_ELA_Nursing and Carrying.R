...
d$adversity.cumul <- d$n_grpsize + d$n_drought + d$n_ibi + d$n_momlossadj + d$Primiparous #2 is momlossadj and original is momloss
...
w <- compare(nursing1,nursing2 , sort=FALSE)@output$weight  ##extract weights from table, alphabetically ordered, important to keep models in alph ordered so proper weights are attributed
...
PM1 <- (1-LM1$p)*LM1$mu #for bndur_M32
PM2 <- (1-LM2$p)*LM2$mu #for bndur_M3adv

#make a vector or predictions sampled according to model weight, should be ordereed alphabetically/numerically
pred_1 <- PM1 #makes pred nurse all predictions from bndur_M32
pred_1[1:idw[2],] <- PM2[1:idw[2],] #overwrite predfather with approriate number of weighted predicitons from PF3a

pred1 <- pred_1[sample(nrow(pred_1)),] #randomly mixes vector-- important to make graphing individual model predictions random
...
#generate predictions from each model that will be averaged
PM1a <- (1-LM1a$p)*LM1a$mu #for bndur_M32
PM2a <- (1-LM2a$p)*LM2a$mu #for bndur_M3adv
...
d.pred2 <- list(
  ...
  challenge=rep(mean(d$s_challenges),length(age.seq)),
  grass=rep(mean(d$s_grass),length(age.seq)),
  mom=rep(mean(d$mom.),length(age.seq)),
  grpsize=rep(mean(d$s_grpsize_cur),length(age.seq)),
  momage=rep(mean(d$s_momage),length(age.seq)),
  ageop=rep(mean(d$s_ageopuntia),length(age.seq)),
  age=rep(mean(d$s_age),length(age.seq)),
...
#w <- compare(nursing1b,nursing2b , sort=FALSE)@output$weight  ##extract weights from table, alphabetically ordered, important to keep models in alph ordered so proper weights are attributed
#w <- w/sum(w)  ##make sure everything adds up to 1
#idw <- round( w * 1000 ) ###round weights to nearest integer so samples we extract sum up to 1000

#generate predictions from each model that will be averaged
PM1b <- (1-LM1b$p)*LM1b$mu #for bndur_M32
PM2b <- (1-LM2b$p)*LM2b$mu #for bndur_M3adv
...
w <- compare(carrying1,carrying2 , sort=FALSE)@output$weight  ##extract weights from table, alphabetically ordered, important to keep models in alph ordered so proper weights are attributed
w <- w/sum(w)  ##make sure everything adds up to 1
idw <- round( w * 1000 ) ###round weights to nearest integer so samples we extract sum up to 1000
...
#w <- compare(carrying1b,carrying2b , sort=FALSE)@output$weight  ##extract weights from table, alphabetically ordered, important to keep models in alph ordered so proper weights are attributed
#w <- w/sum(w)  ##make sure everything adds up to 1
#idw <- round( w * 1000 ) ###round weights to nearest integer so samples we extract sum up to 1000
...
w <- compare(carrying1_separate,carrying2_separate , sort=FALSE)@output$weight  ##extract weights from table, alphabetically ordered, important to keep models in alph ordered so proper weights are attributed
w <- w/sum(w)  ##make sure everything adds up to 1
idw <- round( w * 1000 ) ###round weights to nearest integer so samples we extract sum up to 1000
